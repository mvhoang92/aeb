...
> Bản v2: bổ sung đánh giá tái lập (repeatability), so sánh định lượng radar-only
> vs camera-gated fusion, bộ kiểm thử false-positive vật lý, phân tích giới hạn
> camera gate, ablation bộ điều khiển phanh và phân tích độ nhạy tham số.
...
Ngoài kết quả trên, đồ án bổ sung thêm một lớp đánh giá so sánh giữa hai chiến
lược cảm biến: radar-only và camera-gated fusion, trên cùng một harness kiểm
thử, cùng bộ kịch bản, cùng bộ điều khiển staged PID và cùng số lần lặp. Kết quả
cho thấy camera gate loại bỏ toàn bộ phanh nhầm do radar khi gặp vật thể không
phải ô tô nằm gần mép đường (40/40 phanh nhầm của radar-only bị loại bỏ), đồng
thời không làm tăng đáng kể độ trễ kích hoạt phanh trên mục tiêu ô tô (trung vị
0 ms). Tuy nhiên, vì gate chỉ xác nhận lớp `car`, hệ thống fusion bỏ sót toàn bộ
vật cản không phải ô tô nằm ngay giữa làn đường (10/10 va chạm), trong khi
radar-only dừng an toàn (10/10). Đây là trade-off precision/recall được định
lượng rõ ràng và là đóng góp chính của phiên bản đánh giá bổ sung này.

**Từ khóa:** AEB, ADAS, CARLA, radar, YOLO, hợp nhất camera-radar, TTC,
điều khiển phanh staged PID, precision-recall, repeatability, ablation.
...
Kết quả cho thấy camera gate không làm tăng đáng kể độ trễ kích hoạt phanh trên
mục tiêu ô tô đã được xác nhận: trung vị chênh lệch là 0 ms. Nói cách khác, lợi
ích precision của fusion không phải trả bằng chi phí trễ phản ứng trên xe thật.
...
Bộ props vật lý gồm `static.prop.trafficcone01`, `static.prop.barrel`,
`static.prop.box01`, `static.prop.trashcan01`, `static.prop.streetbarrier` đặt
ở mép đường (offset 1.30--1.50 m). Radar-only chọn các return này làm mục tiêu
...
Phiên bản đánh giá bổ sung còn bổ sung thêm:

- chạy lại full-suite 66 kịch bản với repeat x5 và boundary probe x10 để kiểm
  tra tính tái lập của kết quả;
- so sánh định lượng radar-only và camera-gated fusion bằng ma trận nhầm lẫn,
  cho thấy fusion đạt precision 1.000 (loại bỏ toàn bộ phanh nhầm) với độ trễ
  kích hoạt phanh gần như không đổi (trung vị 0 ms);
- xây dựng bộ kiểm thử false-positive vật lý bằng các props CARLA gần mép đường
  và bộ kiểm thử limitation non-vehicle ngay giữa làn;
- ablation bộ điều khiển binary vs staged_pid, chỉ ra staged_pid phanh sớm hơn
  nhờ ngưỡng margin khoảng cách dừng;
- phân tích độ nhạy `confirmation_hold_s` và phân tích edge case
  `cut_out_late_65_35`.
...
Về vai trò của camera gate, kết luận được rút gọn thành một trade-off precision/recall
được định lượng rõ. Camera gate là cơ chế giảm phanh nhầm: nó loại bỏ toàn bộ
phanh nhầm của radar trên vật thể không phải ô tô gần mép đường mà không làm chậm
phanh trên mục tiêu ô tô. Tuy nhiên, vì gate chỉ xác nhận lớp `car`, nó bỏ sót
vật cản không phải ô tô nằm ngay giữa làn. Vì vậy, trong phạm vi đồ án, fusion
nên được hiểu là bộ lọc false-positive có độ chính xác cao, không phải bộ tránh
vật cản tổng quát, và cần có fallback (ví dụ mở rộng lớp đối tượng hoặc cảnh báo
bổ sung) nếu triển khai thực tế.
