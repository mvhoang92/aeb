...
\begin{abstract}
A car-only camera detector can suppress radar-triggered false braking, but
failure to recognize a vehicle does not establish free road. We evaluate three
late brake-permission policies---radar-only, hard car-camera gating, and car
camera gating with radar emergency fallback---in a shared sensor-to-brake CARLA
pipeline. The primary unit is a
...
3.60-s braking and 9.02~m/s$^2$ median peak deceleration. A previously unused
hold-out bench condition exposes the opposite cost: last-tick pre-impact speed is 32.57~km/h for radar-only,
...
interleaved across splits, but all data remain Town04 in-domain. The detector
metrics reported here are validation metrics from model selection: precision,
recall, $\mathrm{mAP}_{50}$, and $\mathrm{mAP}_{50:95}$ are 0.9919, 0.9642,
0.9940, and 0.9495. The separate test split is retained in the dataset audit;
neither split establishes out-of-domain AEB performance. Calibrated extrinsics project the selected radar target into
...
\quad m=d-d_{\rm req},
\end{equation}
where $t_r=0.20$ s, $a_e=8$, $a_t=6$ m/s$^2$, and $d_0=1$ m are nominal
simulation risk parameters. Later peak decelerations are logged closed-loop
outcomes, not constraints imposed by $a_e$. A shared staged PID supplies
continuous brake commands.
...
perturbation use two and three, respectively. Seed 2026 is recorded. No named
condition produced mixed PASS/FAIL outcomes across its repeats: reported
condition-level system outcomes are all-PASS or all-FAIL. Repeated runs therefore
measure consistency, not independent traffic encounters.
...
\begin{table*}[t]
\caption{Primary named-condition results. Bracketed ranges are descriptive Wilson intervals on the constructed grid only; Coll.: collision conditions.}
\label{tab:primary}
\centering\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{llrrrrrccrr}
\toprule Scope & Policy & N & TP & FP & TN & FN & Precision [range] & Recall [range] & PASS & Coll.\\
\midrule
...
\caption{Frozen named-condition evidence. Left: core precision--recall with descriptive Wilson ranges on constructed named conditions. Right: system PASS on the 14-condition adverse hold-out. Neither grid estimates road prevalence.}
\label{fig:primary}
\end{figure*}
...
Source code, manuscript files, checksums, and curated derived evidence are indexed at
\url{https://github.com/mvhoang92/aeb}; the companion source map identifies the
frozen raw archive, derivation scripts, and checksum trail. Large runtime artifacts
and local model files are referenced by hash and path policy rather than embedded
in the manuscript package. Re-execution requires a compatible CARLA installation,
local model artifact, and CUDA environment.
