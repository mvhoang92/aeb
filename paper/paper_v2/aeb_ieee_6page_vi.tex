...
\maketitle
\begin{abstract}
Phanh khẩn cấp tự động là một bài toán vòng kín: hệ thống phải biến dữ liệu cảm biến còn nhiễu thành một mục tiêu có liên quan va chạm, sau đó tạo lệnh phanh đủ sớm mà không phản ứng nhầm với xe làn bên. Bài báo trình bày một pipeline AEB theo hướng radar-first, camera-verified trong CARLA 0.9.11. Radar phía trước là nguồn đo chính cho khoảng cách, vận tốc tương đối, TTC và khoảng cách dừng; các điểm radar thưa được chuyển thành object/track thông qua lọc, gom cụm, tóm tắt cụm bền vững và xác nhận đa khung hình. Camera RGB và YOLO26n được dùng như tầng xác nhận ngữ nghĩa: mục tiêu radar được chiếu lên ảnh và chỉ được phép kích hoạt phanh khi liên kết hợp lệ với bounding box lớp xe. Trên cơ sở mục tiêu đã chọn, hệ thống dùng hành lang quỹ đạo dự đoán, TTC, biên khoảng cách dừng và bộ điều khiển staged PID để tạo lệnh phanh. Tập evidence 66 kịch bản gồm CCRm, CCRb và cut-in cho thấy hệ thống đạt 38/38 trường hợp trong miền thiết kế, 25/28 trường hợp stress test và 63/66 trường hợp tổng cộng. Ba va chạm được giữ lại để phân tích giới hạn ở tốc độ cao, gap ngắn và cut-in muộn. Các kết quả này là bằng chứng kỹ thuật trong mô phỏng, không phải chứng nhận NCAP hoặc kiểm định trên xe thật.
\end{abstract}
\begin{IEEEkeywords}
...
Đóng góp của bài báo gồm: 1) tầng radar mức đối tượng cho CARLA AEB, trong đó điểm radar được lọc, gom cụm, theo dõi và xác nhận trước khi có quyền kích hoạt phanh; 2) cơ chế radar-first, camera-verified bằng cách chiếu mục tiêu radar đã chọn vào bounding box YOLO mà không dùng actor ID hoặc ground truth CARLA khi quyết định online; 3) chuỗi đánh giá rủi ro theo hành lang quỹ đạo, TTC, biên khoảng cách dừng và staged PID; và 4) cách đánh giá tách miền thiết kế với stress test, trong đó các case fail tốc độ cao, gap ngắn và cut-in muộn được giữ lại để phân tích giới hạn.
...
Tập evidence cuối gồm 66 case dùng cho kết quả tổng hợp trong bài báo: 24 CCRm, 30 CCRb và 12 cut-in. Các case đường trống, CCRs, xe làn bên, đường cong, cut-out và nhiều actor được dùng như các kiểm tra bổ sung trong quá trình phát triển, nhưng không gộp vào tỷ lệ 63/66. Bảng~\ref{tab:scope} phân tách hai lớp đánh giá. Nhóm đầu kiểm tra dải hoạt động mục tiêu; nhóm sau mở rộng tốc độ, gap và độ khó để tìm giới hạn, không nhằm làm đẹp tỷ lệ pass.
...
Bảng~\ref{tab:scenariofamilies} mô tả các nhóm scenario chính trong toàn bộ quá trình phát triển và đánh giá. Trong đó, kết quả tổng hợp 66 case của bài báo chỉ lấy CCRm, CCRb và cut-in; các nhóm còn lại đóng vai trò kiểm tra bổ sung về false positive và chọn target.
...
Bảng~\ref{tab:cases} liệt kê một số case đại diện từ log final và các kiểm tra bổ sung. Các giá trị ``brake speed'' và ``brake gap'' được lấy tại thời điểm lệnh phanh bắt đầu xuất hiện trong log. Min gap là khoảng cách nhỏ nhất trong toàn bộ scenario. Các case này giúp đọc hành vi hệ thống ở xe chạy chậm hơn, xe trước phanh gấp, target nhập làn, đường cong và nhiều actor.
