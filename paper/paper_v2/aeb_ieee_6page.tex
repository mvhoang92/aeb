...
\title{A Closed-Loop AEB Pipeline in CARLA Using Object-Level Radar, YOLO Verification, and Staged PID Braking}
...
\begin{abstract}
Automatic emergency braking (AEB) is a closed-loop problem: the system must
turn noisy sensor measurements into a collision-relevant target and then brake
early enough without reacting to adjacent traffic. This paper presents a
radar-first, camera-verified AEB pipeline in CARLA 0.9.11 for highway
car-to-car scenarios. Sparse simulated radar returns are converted into object
tracks by filtering, clustering, robust cluster summarization, and multi-frame
confirmation. A YOLO26n detector, fine-tuned for the CARLA windshield-camera
view, is used only as semantic verification: the selected radar object is
projected into the image and allowed to trigger braking only when it is
associated with a detected car box. A predicted ego-path corridor,
time-to-collision (TTC), stopping-distance margin, and staged PID controller
then produce the brake command. In 66 controlled simulations, the system passes
all 38 intended-ODD cases, mainly 50--80 km/h in ideal weather, and passes 63
cases overall. The three retained failures expose high-speed, short-gap braking
and late cut-in boundaries. The study is positioned as a reproducible CARLA
engineering baseline, not as an NCAP certification or real-vehicle validation.
\end{abstract}
...
forward radar mounted on a Tesla Model 3. The focus is highway car-to-car
operation inspired by the stationary, moving, and braking lead-vehicle groups
of Euro NCAP protocols \cite{euroncap_aeb}; the scenarios are engineering tests,
not official certification procedures.

The paper is an integration and validation study rather than a claim of a new
TTC formula, detector, or controller theory. Its contributions are: 1) an
object-level CARLA radar layer that filters, clusters, tracks, and confirms
sparse radar returns before AEB can brake; 2) a radar-first, camera-verified
association gate that projects the selected radar object into YOLO vehicle boxes
without using CARLA actor IDs or online ground truth; 3) a path-aware risk
assessment and staged PID braking chain that combines TTC with stopping-distance
margin; and 4) an ODD-separated, limit-finding evaluation that reports both
successful cases and retained high-speed, short-gap, and cut-in failures.
...
deceleration \cite{iso15623}. Many simulation studies focus on one part of the
chain---for example perception, idealized target state, or a simple brake
threshold. This paper instead emphasizes the closed-loop path from simulated
radar points to a verified target and brake command. Its contribution is the
transparent integration and evaluation of object-level radar processing,
image-space verification, path-aware target selection, and staged PID braking,
not a probabilistic fusion estimator.
...
The final 66-case evidence suite contains 24 moving-lead (CCRm), 30 braking-lead
(CCRb), and 12 cut-in simulations. These are the cases used for the aggregate
numbers in this paper. Additional clear-road, adjacent-lane, curve, cut-out, and
multi-actor runs were used during development, but they are not folded into the
66-case total. Table~\ref{tab:scope} separates the intended ODD from stress
tests, avoiding an unconditional reading of a single aggregate pass rate.
...
The three final scenario groups stress different parts of the closed loop. CCRm
uses a slower lead vehicle, so the closing speed can be much smaller than ego
speed. CCRb introduces a lead vehicle that brakes after both vehicles are
moving. Cut-in tests the moment when a vehicle first becomes relevant to the ego
path. This family-based setup is inspired by car-to-car evaluation structures,
but it does not reproduce the full test apparatus, measurements, or certification
conditions of Euro NCAP.
...
\begin{table*}[t]
\caption{Representative logged scenarios. Brake quantities are sampled at the
first nonzero brake command.}
